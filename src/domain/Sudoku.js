...
export function createSudoku(puzzle) {
  ...
  return {
    ...
    getCandidates(row, col) {
      ...
      // 行
      for (let x = 0; x < SUDOKU_SIZE; x++) {
        if (currentGrid[row][x] !== 0) used.add(currentGrid[row][x]);
      }

      // 列
      for (let y = 0; y < SUDOKU_SIZE; y++) {
        if (currentGrid[y][col] !== 0) used.add(currentGrid[y][col]);
      }
      ...
      for (let y = boxRow; y < boxRow + 3; y++) {
        for (let x = boxCol; x < boxCol + 3; x++) {
          if (currentGrid[y][x] !== 0) used.add(currentGrid[y][x]);
        }
      }
      ...
      for (let y = 0; y < SUDOKU_SIZE; y++) {
        for (let x = 0; x < SUDOKU_SIZE; x++) {
          if (currentGrid[y][x] === 0) {
            const c = this.getCandidates(y, x);
            if (c.length === 1) {
              return { row: y, col: x, value: c[0], candidates: c };
            }
          }
        }
      }
